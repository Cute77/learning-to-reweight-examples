...
def train_net(noise_fraction, 
    ...
    data_loader = DataLoader(train, batch_size=batch_size, shuffle=False, num_workers=8, drop_last=True , pin_memory=True, sampler=train_sampler)
    ...
    global_step = 0
    test_step = 0
    w = torch.ones([batch_size]).cuda(local_rank)
    beta = torch.ones([batch_size]).cuda(local_rank)
    ...
    for epoch in range(load+1, epochs):
        ...
        if epoch % 10 == 0 and local_rank == 0:
            ...
            pickle.dump(ws, open(bdir+'/'+str(load)+"_w.pkl", "wb"))
            pickle.dump(wnoisy, open(bdir+'/'+str(load)+"_wnoisy.pkl", "wb"))
            pickle.dump(wclean, open(bdir+'/'+str(load)+"_wclean.pkl", "wb"))
            print('weight saved')
